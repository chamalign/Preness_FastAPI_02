...
_GRAMMAR_BLANK = "_______"
_GRAMMAR_DOUBLE_COMMA_RE = re.compile(r",\s*,")
_GRAMMAR_TRIPLE_COMMA_RE = re.compile(r",\s*,\s*,")


def _fix_grammar_blank_double_comma(question_text: str, ctx: str) -> str:
    """
    Grammar の question_text に混入する `, ,`（ブランク欠落）を `, _______,` に修正する.

    例: "Echolocation, , allows ..." -> "Echolocation, _______, allows ..."
    """
    if not isinstance(question_text, str):
        raise ValueError(f"question_text must be a string ({ctx})")

    # 連続 3 カンマなどは自動修正が危険なので止める
    if _GRAMMAR_TRIPLE_COMMA_RE.search(question_text):
        raise ValueError(
            f"Invalid Grammar blank marker (triple comma) in question_text ({ctx}): "
            "fix the source txt to include '_______'."
        )

    matches = list(_GRAMMAR_DOUBLE_COMMA_RE.finditer(question_text))
    if not matches:
        return question_text

    # 1 問に複数回出る場合は想定外として止める（誤修正を避ける）
    if len(matches) > 1:
        raise ValueError(
            f"Invalid Grammar blank marker (multiple double commas) in question_text ({ctx}): "
            "fix the source txt to include '_______'."
        )

    fixed = _GRAMMAR_DOUBLE_COMMA_RE.sub(f", {_GRAMMAR_BLANK},", question_text, count=1)
    if _GRAMMAR_DOUBLE_COMMA_RE.search(fixed):
        raise ValueError(
            f"Invalid Grammar blank marker (unfixed) in question_text ({ctx}): "
            "fix the source txt to include '_______'."
        )

    logger.warning(
        "Grammar question_text contained double comma; auto-fixed (%s): %r -> %r",
        ctx,
        question_text,
        fixed,
    )
...
def _patch_question_dict(q: Dict[str, Any], ctx: str, *, is_grammar: bool) -> None:
    ...
    if isinstance(qt, str):
        if is_grammar:
            q["question_text"] = _fix_grammar_blank_double_comma(qt, f"{ctx}.question_text")
            qt = q["question_text"]
...
def normalize_mock_payload_for_rails(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for si, sec in enumerate(sections):
        ...
        is_grammar = section_type == "structure"
        parts = sec.get("parts")
        ...
        for pi, part in enumerate(parts):
            ...
            for qi, qset in enumerate(qsets):
                ...
                for qj, q in enumerate(questions):
                    ...
                    _patch_question_dict(q, ctx, is_grammar=is_grammar)
    return out
...
def normalize_exercise_payload_for_rails(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    out = copy.deepcopy(payload)
    is_grammar = out.get("section_type") == "structure"
